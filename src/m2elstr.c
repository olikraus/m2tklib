...
char *m2_el_str_get_str(m2_el_fnarg_p fn_arg)
{
  return (char *)m2_rom_get_ram_ptr(fn_arg->element, offsetof(m2_el_str_t, str));
  /*return (char *)m2_get_ae_ptr( &(fn_arg->ae), offsetof(m2_el_str_t, str));*/
}


/*
  void *el			pointer to the m2_el_... structure, m2_el_str_p
  uint8_t msg		received message
  void *data		additional data
  
  msg = M2_EL_MSG_IS_CONTAINER
    return value: 0 or 1
...
M2_EL_FN_DEF(m2_el_str_fn)
{
  ...
  switch(fn_arg->msg)
  {
    ...
    case M2_EL_MSG_GET_WIDTH:
      return m2_gfx_add_normal_border_width(font, m2_gfx_get_text_width(font,m2_el_str_get_str(fn_arg)));
#ifdef M2_EL_MSG_DBG_SHOW
	...
	printf("str w:%d h:%d arg:%d x:%d y:%d %s\n", 
	    ...
	    fn_arg->arg, b->x, b->y, m2_el_str_get_str(fn_arg));
      }
      return 0;
#endif
    case M2_EL_MSG_SHOW:
	...
	m2_gfx_draw_text_add_normal_border_offset(b->x, b->y, font, m2_el_str_get_str(fn_arg));
