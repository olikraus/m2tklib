...
struct m2_u8g2_dev_variables_struct
{
  ...
  uint8_t m2_gh_u8g2_invert_at_depth = 255;

  uint8_t m2_gh_u8g2_invisible_frame_border_x_size = 1;
  uint8_t m2_gh_u8g2_additional_text_border_x_size = 1;
  uint8_t m2_gh_u8g2_additional_read_only_border_x_size = 1;

  uint8_t (*m2_gh_u8g2_icon_draw)(m2_gfx_arg_p  arg) = m2_gh_dummy;
};
*/

struct m2_u8g2_dev_variables_struct m2_u8g2_dev_variables =
{
  NULL, 0, 0, 0, 0, 0, 
  0, 255, 1, 1, 1, 
  m2_gh_dummy
...
{
  m2_u8g2_dev_variables.m2_gh_u8g2_invisible_frame_border_x_size = w;
}

void m2_SetU8g2AdditionalTextXBorder(uint8_t w)
{
  m2_u8g2_dev_variables.m2_gh_u8g2_additional_text_border_x_size = w;
}
...
{
  m2_u8g2_dev_variables.m2_gh_u8g2_additional_read_only_border_x_size = w;
}
...
void m2_u8g2_draw_frame(uint8_t x0, uint8_t y0, uint8_t w, uint8_t h)
{
  ...
  u8g2_SetDrawColor(m2_u8g2_dev_variables.m2_u8g2, m2_u8g2_dev_variables.m2_u8g2_draw_color);
  u8g2_DrawFrame(m2_u8g2_dev_variables.m2_u8g2, x0, y, w, h);
  /*
...
void m2_u8g2_draw_frame_shadow(uint8_t x0, uint8_t y0, uint8_t w, uint8_t h)
{
  ...
  u8g2_SetDrawColor(m2_u8g2_dev_variables.m2_u8g2, m2_u8g2_dev_variables.m2_u8g2_draw_color);
  u8g2_DrawFrame(m2_u8g2_dev_variables.m2_u8g2, x0, y, w, h);
  u8g2_DrawVLine(m2_u8g2_dev_variables.m2_u8g2, x0+w, y+1, h);
  u8g2_DrawHLine(m2_u8g2_dev_variables.m2_u8g2, x0+1, y+h, w);
}
...
void m2_u8g_draw_box(uint8_t x0, uint8_t y0, uint8_t w, uint8_t h)
{
  ...
  y++;			/* 13 Jan 2013: Issue 95 */
  u8g2_SetDrawColor(m2_u8g2_dev_variables.m2_u8g2, m2_u8g2_dev_variables.m2_u8g2_draw_color);
  u8g2_DrawBox(m2_u8g2_dev_variables.m2_u8g2, x0, y, w, h);
}



const uint8_t *m2_gh_u8g_fonts[4];
...
uint8_t m2_gh_u8g_base(m2_gfx_arg_p  arg)
{
  ...
  if ( m2_u8g2_dev_variables.m2_u8g2 == NULL )
    return 0;
  ...
  switch(arg->msg)
  {
    ...
    case M2_GFX_MSG_DRAW_HLINE:
      u8g2_DrawHLine(m2_u8g2_dev_variables.m2_u8g2, arg->x, m2_u8g2_dev_variables.m2_u8g2_height_minus_one - arg->y, arg->w);
      break;
    case M2_GFX_MSG_DRAW_VLINE:
      u8g2_DrawVLine(m2_u8g2_dev_variables.m2_u8g2, arg->x, m2_u8g2_dev_variables.m2_u8g2_height_minus_one - arg->y, arg->h);  
      break;
    ...
    case M2_GFX_MSG_DRAW_TEXT:
      {
        ...
        if ( (arg->font & 8) != 0 )
        {
          ...
          {
            x = arg->w;
            x -= u8g2_GetStrWidth(m2_u8g2_dev_variables.m2_u8g2, arg->s);
            x >>= 1;
            x += arg->x;
          }
        }
	...
	y++;			/* 13 Jan 2013: Issue 95, problem 2 */
        x -= u8g2_GetStrX(m2_u8g2_dev_variables.m2_u8g2, arg->s);
        //x += m2_gh_u8g_additional_text_border_x_size;
        u8g2_DrawStr(m2_u8g2_dev_variables.m2_u8g2, x, y, arg->s);
	...
	u8g_uint_t y;
	      ...
	      if ( (arg->font & 8) != 0 )
        {
	        ...
	        {
	          x = arg->w;
	          x -= u8g2_GetStrWidthP(m2_u8g2_dev_variables.m2_u8g2, (const u8g_pgm_uint8_t *)arg->s);
	          x >>= 1;
	          x += arg->x;
	        }
        }
        y = m2_u8g2_dev_variables.m2_u8g2_height_minus_one;
      	y -= arg->y;
        x -= u8g_GetStrXP(m2_u8g2_dev_variables.m2_u8g2, (const u8g_pgm_uint8_t *)arg->s);
        //x += m2_gh_u8g_additional_text_border_x_size;
	y++;			/* 13 Jan 2013: Issue 95, problem 2 */
    ...
    case M2_GFX_MSG_DRAW_VERTICAL_SCROLL_BAR:
      ...
      m2_u8g2_draw_frame(arg->x, arg->y, arg->w, arg->h);
      {
      	uint16_t h, y;
      	...
      	m2_u8g2_draw_box(arg->x+1, arg->y+arg->h-1-h-y, arg->w-2, h);
      }
      return 1;
    ...
    case M2_GFX_MSG_GET_READONLY_BORDER_WIDTH:
      return 2*m2_u8g2_dev_variables.m2_gh_u8g2_additional_read_only_border_x_size;
    case M2_GFX_MSG_GET_READONLY_BORDER_X_OFFSET:
      return m2_u8g2_dev_variables.m2_gh_u8g2_additional_read_only_border_x_size;
    ...
    */
	...
	y -= arg->h;
	y++;			/* 13 Jan 2013: Issue 95 */
      	u8g2_DrawXBMP(m2_u8g2_dev_variables.m2_u8g2, x, y, arg->w, arg->h, (const char *)arg->s);
      }
  }

  return m2_u8g2_dev_variables.m2_gh_u8g2_icon_draw(arg);
}
